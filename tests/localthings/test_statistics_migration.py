...
def _recorder_loaded(hass: HomeAssistant):
    ...
    pulled the recorder in. test_is_a_no_op_without_the_recorder undoes it."""
...
async def test_is_a_no_op_without_the_recorder(hass: HomeAssistant) -> None:
    """A setup running without the recorder has no statistics to relabel,
    and importing recorder internals there would fail outright."""
    entry = _entry(hass, "air_purifier")
    _add_sensor(hass, entry, "dust")
...
async def test_matches_subdevice_prefixed_and_instanced_keys(hass: HomeAssistant) -> None:
    ...
    match is on the tail rather than the whole unique_id."""
    entry = _entry(hass, "air_purifier")
    ent_reg = er.async_get(hass)
    for unique_suffix in ("indoor_0_dust", "fine_dust1", "super_fine_dust"):
